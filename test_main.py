...
from main import (
    app, db, courses_db, 
    normalize_code, format_code_for_msg, 
    term_sort_key, extract_course_codes
)
...
def test_import_catalog_no_table():
    response = client.post(
        "/api/v1/admin/catalog/import",
        files={"file": ("bad.html", "<html><body>No data here</body></html>", "text/html")}
    )
...
def test_student_profile_crud():
    ...
    # Test PUT History
    h_payload = {"history": [{"course_code": "MATH1000", "term": "23F", "credits_earned": 3, "status": "Completed"}]}
    assert client.put("/api/v1/students/S999/history", json=h_payload).status_code == 200
    assert client.put("/api/v1/students/NOBODY/history", json=h_payload).status_code == 404
    ...
    assert client.post("/api/v1/students/NOBODY/plan", json=p_payload).status_code == 404
...
def test_audit_report():
    ...
    db["S_AUDIT"] = {
        "history": [
            ...
            {"course_code": "COSC 1000", "term": "23F", "credits_earned": 3, "status": "Completed"},
            # Passed the cross-listed equivalent
            {"course_code": "MATH 2000", "term": "23F", "credits_earned": 3, "status": "Completed"}
        ],
        "plan": [
