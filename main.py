...
async def import_history(student_id: str, file: UploadFile = File(...)):
    ...
    for table in soup.find_all('table'):
        ...
        if len(headers) < 6: continue
        
        header_texts = [th.get_text(separator=" ", strip=True).lower() for th in headers]
        if not ('status' in header_texts[0] and 'course' in header_texts[1] and 
                'grade' in header_texts[3] and 'term' in header_texts[4] and 'credits' in header_texts[5]):
            continue
        ...
        for row in tbody.find_all('tr'):
            ...
            if 'Completed' in raw_status: c_status = 'Completed'
            elif 'In-Progress' in raw_status: c_status = 'In-Progress'
            elif 'Attempted' in raw_status: c_status = 'Attempted'
            else: continue 
            ...
            try: credits_earned = int(raw_credits)
            except ValueError: credits_earned = 0
            ...
            try:
                ...
                if raw_grade.upper() not in ['P', '']: grade_rank = 2
                else: grade_rank = 1
            ...
            if key in extracted:
                ...
                if grade_rank > existing['grade_rank'] or (grade_rank == existing['grade_rank'] and credits_earned > existing['credits_earned']):
                    extracted[key] = {'course': new_entry, 'grade_rank': grade_rank, 'credits_earned': credits_earned}
            else:
                extracted[key] = {'course': new_entry, 'grade_rank': grade_rank, 'credits_earned': credits_earned}
...
def update_history(student_id: str, payload: HistoryUpdate):
    if student_id not in db: raise HTTPException(status_code=404, detail="Student not found")
...
def get_audit_report(student_id: str, strict: bool = False):
    ...
    history = sorted(student_data.get("history", []), key=lambda c: term_sort_key(c["term"]))
    ...
    for pc in planned:
        ...
        if cat_info:
            ...
            for pq in prereq_codes:
                ...
                if pq not in completed_terms or term_sort_key(completed_terms[pq]) >= term_sort_key(p_term):
